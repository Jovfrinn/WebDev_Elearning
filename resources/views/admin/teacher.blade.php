...
@section('pageTitle', 'Data Pengajar')
...
<div class="max-w-6xl mx-auto">
    ...
    <!-- Success Flash -->
    @if(session('success'))
    <div id="flash-success" class="mb-6 flex items-center gap-3 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl px-4 py-3 shadow-sm">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Data Pengajar</h2>
            <p class="text-sm text-slate-500 mt-1">Daftar seluruh pengajar yang aktif di platform.</p>
        </div>
        <div class="flex items-center gap-3 flex-wrap">
            <span class="inline-flex items-center px-4 py-2 bg-rose-50 text-rose-700 rounded-full text-sm font-semibold border border-rose-100">
                <span class="material-symbols-outlined text-[18px] mr-1.5">school</span>
                {{ $count }} Pengajar
            </span>
            <a href="{{ route('create.teacher') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white text-sm font-semibold rounded-xl shadow-md shadow-rose-200 hover:shadow-rose-300 transition-all duration-200 group">
                <span class="material-symbols-outlined text-[18px] mr-1.5 group-hover:scale-110 transition-transform">person_add</span>
                Tambah Guru
            </a>
        </div>
    </div>
    ...
    <div class="mb-6">
        <div class="relative max-w-md">
            ...
            <input type="text" id="searchInput" placeholder="Cari nama, email, atau NIP pengajar..."
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr>
                        <td colspan="6" class="py-12 px-6 text-center">
                            ...
                            <p class="font-medium text-slate-600">Belum ada pengajar terdaftar.</p>
                        </td>
                    </tr>
                    @else
                    @php $no = 1; @endphp
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors searchable-row">
                        ...
                        <td class="py-4 px-6">
                            <div class="h-10 w-10 rounded-full overflow-hidden border-2 border-slate-100 shadow-sm">
                                ...
                                @else
                                <img src="{{ asset('assets/img/'.$teacher->image_profile) }}" alt="{{ $teacher->name }}" class="h-full w-full object-cover">
                                @endif
                            </div>
                        </td>
                        <td class="py-4 px-6">
                            <p class="text-sm font-semibold text-slate-800 searchable-name">{{ $teacher->name }}</p>
                        </td>
                        <td class="py-4 px-6">
                            <p class="text-sm text-slate-600 searchable-email">{{ $teacher->email }}</p>
                        </td>
                        <td class="py-4 px-6">
                            <span class="px-2.5 py-1 bg-slate-100 text-slate-700 text-xs font-mono font-semibold rounded-md searchable-nip">{{ $teacher->nip }}</span>
                        </td>
                        <td class="py-4 px-6 text-center">
                            <button onclick="return confirm('Apakah Anda yakin ingin menghapus pengajar ini?')" class="px-4 py-2 border border-red-200 text-red-600 text-xs font-semibold rounded-lg hover:bg-red-50 hover:border-red-300 transition-colors inline-flex items-center">
    ...
    <div id="pagination-wrap" class="hidden mt-6">
        <div class="flex justify-center">
            <nav id="pagination-nav" class="inline-flex items-center gap-1" aria-label="Pagination Pengajar"></nav>
        </div>
