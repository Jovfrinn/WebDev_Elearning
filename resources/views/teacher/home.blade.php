...
@else
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <a href="{{route('add.materi')}}"
                class="inline-flex items-center justify-center px-6 py-3 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-all duration-200 shadow-md shadow-emerald-200 group">
    ...
    <div class="bg-white rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100 overflow-hidden">
        @if($materials->isEmpty())
            ...
            <div class="p-12 text-center flex flex-col items-center justify-center">
                ...
                <p class="text-slate-500 max-w-sm mb-6">Mulai perjalanan mengajar Anda dengan membuat kelas pertama. Bagikan
                    pengetahuan Anda kepada dunia!</p>
        ...
        @else
            ...
            <ul class="divide-y divide-slate-100">
                @foreach ($materials as $material)
                    <li
                        class="p-4 sm:p-6 hover:bg-slate-50/50 transition-colors group flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div class="flex items-start sm:items-center gap-4">
                            <!-- Thumbnail -->
                            <div
                                class="h-16 w-24 sm:h-20 sm:w-32 rounded-xl overflow-hidden shrink-0 bg-slate-100 border border-slate-200 relative">
                                <img src="{{asset('assets/img/' . $material->material_image)}}" alt="Thumbnail"
                            ...
                            <div>
                                <a href="{{route('get.subMateri', $material->id)}}"
                                    class="text-lg font-bold text-slate-800 hover:text-emerald-600 transition-colors line-clamp-1 mb-1">
                        ...
                        <div class="flex items-center gap-2 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('teacher.analytics', $material->id) }}"
                            ...
                            <a href="#"
                                class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors tooltip"
                                title="Hapus Kelas">
