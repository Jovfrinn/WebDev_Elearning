...
@section('title', 'Daftar Guru - EduVortex')
...
@section('content')
    ...
    <div class="bg-white/80 backdrop-blur-xl border border-white/60 shadow-2xl shadow-indigo-200/50 rounded-3xl p-8 sm:p-10 w-full max-w-md mx-auto">
        ...
        <div class="text-center mb-8">
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Buat Akun Guru Baru</h2>
            <p class="text-sm text-slate-500 mt-2">Mulai perjalanan mengajarmu bersama kami.</p>
        </div>
        ...
        <form method="POST" action="{{ route('register.teacher') }}" class="space-y-5">
            ...
            <!-- NIP -->
            <div>
                <label for="nip" class="block text-sm font-medium text-slate-700 mb-1">Nomor Induk Pegawai (NIP)</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    ...
                    <input type="text" id="nip" name="nip" :value="old('nip')" required 
                        class="block w-full pl-10 pr-3 py-2.5 border border-slate-200 rounded-xl bg-white/50 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition-colors text-slate-800 placeholder-slate-400" placeholder="1234567890">
                </div>
